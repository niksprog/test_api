...
class ApiThrottler
{
     ...
     */
    public function handle($request, Closure $next, $maxAttempts = 5, $decayMinutes = 60)
    {
    ...
    {
        if($request->routeIs('api.vessels.index'))
        {
            return 'vessels';
        }

        if($request->routeIs('api.reports.index'))
        {
            return 'reports';
        }
    ...
    {
        if($request->get('format')=='csv') return 'csv';
